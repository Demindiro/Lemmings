...
macro_rules! available_ring {
    ($self:ident) => {
        unsafe { return_ring::<Avail, AvailHead, AvailElement>($self.available, $self.mask) }
    };
}
...
macro_rules! used_ring {
    ($self:ident) => {
        unsafe { return_ring::<Used, UsedHead, UsedElement>($self.used, $self.mask) }
    };
}
...
macro_rules! descriptors_table {
    ($self:ident) => {
        unsafe { return_table::<Descriptor>($self.descriptors, $self.mask) }
    };
}
...
impl<'a> Queue<'a> {
    ...
    /// # Safety
    ///
    /// `dma_alloc` **must** return physical addresses aligned to 4096 bytes.
    // TODO 4096 only applies to the legacy interface (page alignment).
    // the actual required alignment is only 16. Update?
        ...
        F: FnOnce(usize) -> Result<(NonNull<u8>, PhysAddr), E>,
    {
        ...
        let (mem, phys) = dma_alloc(align(desc_size + avail_size) + align(used_size))
            .map_err(NewQueueError::DmaError)?;

        let descriptors = mem.cast();
        let available = unsafe { mem.byte_add(desc_size).cast() };
        let used = unsafe { mem.byte_add(align(desc_size + avail_size)).cast::<Used>() };
        ...
        I: ExactSizeIterator<Item = (PhysAddr, u32, bool)>,
    {
        ...
        let count = u16::try_from(count).expect("too many elements");
...
unsafe fn return_ring<'s, R, H, E>(ptr: NonNull<R>, mask: u16) -> (&'s mut H, &'s mut [E]) {
    unsafe {
        let size = usize::from(mask) + 1;
        let head = &mut *ptr.as_ptr().cast::<H>();
        let ring = ptr.as_ptr().byte_add(mem::size_of::<H>());
...
/// Returns the table
unsafe fn return_table<'s, T>(ptr: NonNull<T>, mask: u16) -> &'s mut [T] {
    unsafe {
        let size = usize::from(mask) + 1;
